...
func TestGetByCollectionSortedWithKeysetCursor_BadgeAwardBadgeTypeFilter(t *testing.T) {
	...
	insertSortRecord(t, repo, endorsementBadgeURI, "cid-endorsement", "did:plc:issuer", "app.certified.badge.definition", `{"title":"Endorsement","badgeType":"endorsement"}`)
	insertSortRecord(t, repo, trustedBadgeURI, "cid-trusted", "did:plc:issuer", "app.certified.badge.definition", `{"title":"Trusted","badgeType":"trusted-evaluator"}`)
	insertSortRecord(t, repo, missingTypeBadgeURI, "cid-missing-type", "did:plc:issuer", "app.certified.badge.definition", `{"title":"Missing type"}`)

	insertSortRecord(t, repo, "at://did:plc:issuer/app.certified.badge.award/endorsement", "cid-award-endorsement", "did:plc:issuer", "app.certified.badge.award", `{"badge":{"uri":"`+endorsementBadgeURI+`","cid":"cid-endorsement"},"subject":{"$type":"app.certified.defs#did","did":"did:plc:alice"}}`)
	insertSortRecord(t, repo, "at://did:plc:issuer/app.certified.badge.award/trusted", "cid-award-trusted", "did:plc:issuer", "app.certified.badge.award", `{"badge":{"uri":"`+trustedBadgeURI+`","cid":"cid-trusted"},"subject":{"$type":"app.certified.defs#did","did":"did:plc:bob"}}`)
	insertSortRecord(t, repo, "at://did:plc:issuer/app.certified.badge.award/missing-type", "cid-award-missing-type", "did:plc:issuer", "app.certified.badge.award", `{"badge":{"uri":"`+missingTypeBadgeURI+`","cid":"cid-missing-type"},"subject":{"$type":"app.certified.defs#did","did":"did:plc:carol"}}`)
	insertSortRecord(t, repo, "at://did:plc:issuer/app.certified.badge.award/missing-definition", "cid-award-missing-definition", "did:plc:issuer", "app.certified.badge.award", `{"badge":{"uri":"at://did:plc:issuer/app.certified.badge.definition/missing","cid":"cid-missing"},"subject":{"$type":"app.certified.defs#did","did":"did:plc:dana"}}`)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			records, err := repo.GetByCollectionSortedWithKeysetCursor(ctx, "app.certified.badge.award", []FieldFilter{
				{Field: "badgeType", Operator: tt.operator, Value: tt.value, FieldType: "string", Target: FieldFilterTargetBadgeAwardBadgeType},
			}, DIDFilter{}, nil, 10, nil)
			if err != nil {
				t.Fatalf("GetByCollectionSortedWithKeysetCursor() error = %v", err)
			}
			...
			count, err := repo.GetCollectionCountFiltered(ctx, "app.certified.badge.award", []FieldFilter{
				{Field: "badgeType", Operator: tt.operator, Value: tt.value, FieldType: "string", Target: FieldFilterTargetBadgeAwardBadgeType},
			}, DIDFilter{})
			if err != nil {
				t.Fatalf("GetCollectionCountFiltered() error = %v", err)
...
func assertRecordURISet(t *testing.T, records []*Record, wantURIs []string) {
	t.Helper()
	if len(records) != len(wantURIs) {
		t.Fatalf("len(records) = %d, want %d; got %v", len(records), len(wantURIs), recordURIs(records))
	}
	...
	for _, record := range records {
		if !want[record.URI] {
			t.Fatalf("unexpected record URI %q; want set %v", record.URI, wantURIs)
		}
	}
}

func recordURIs(records []*Record) []string {
