...
func (r *RecordsRepository) EndorsementAdjacencyFor(ctx context.Context, issuers []string) (map[string][]string, error) {
	...
	for start := 0; start < len(issuers); start += batchSize {
		...
		batchAdjacency, err := r.endorsementAdjacencyBatch(ctx, issuers[start:end])
		if err != nil {
			return nil, err
		}
...
func (r *RecordsRepository) endorsementAdjacencyBatch(ctx context.Context, issuers []string) (map[string][]string, error) {
	...
	ORDER BY endorsement_awards.issuer_did, endorsement_awards.subject_did`,
		subjectDIDExpr,
		badgeURIExpr,
	...
	if err != nil {
		return nil, fmt.Errorf("query Certified endorsement adjacency: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&issuer, &subject); err != nil {
			return nil, fmt.Errorf("scan Certified endorsement adjacency row: %w", err)
		}
		out[issuer] = append(out[issuer], subject)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate Certified endorsement adjacency rows: %w", err)
	}
