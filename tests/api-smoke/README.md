...
The default expectations only assert `app.certified.*` and `org.hypercerts.*` Lexicons. Current Hyperindex binaries include those schemas and their helper dependencies in the CID-pinned bundle. Use `LEXICON_DIR` only to test schema overrides, or point `HYPERINDEX_SMOKE_EXPECTATIONS` at an environment-specific expectations file when testing an older or customized deployment.
